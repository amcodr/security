...
//				principalExtractor, configPath, threadPool);
	...
	protected void handleDelete(final RestChannel channel, final RestRequest request, final Client client,
								final Builder additionalSettingsBuilder) throws IOException {
	...
	protected void handlePost(final RestChannel channel, final RestRequest request, final Client client,
							  final Builder additionalSettings) throws IOException {
	...
	protected final Tuple<Long, Builder> load(final String config, boolean logComplianceEvent) {
		Tuple<Long, Settings> t = loadAsSettings(config, logComplianceEvent);
		return new Tuple<Long, Builder>(t.v1(), Settings.builder().put(t.v2()));
	}
	...
	protected void filter(Builder builder) {
	...
	protected void saveAnUpdateConfigs(final RestChannel channel, final Client client, final RestRequest request, final String config,
									   final Builder settings, OnSucessActionListener<IndexResponse> actionListener, long version) {
...
//		}
	...
	protected static BytesReference toSource(RestChannel channel, final Builder settingsBuilder) { //not throws
		try {
